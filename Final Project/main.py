...
# --- 1. ALGORITMA CORE (ALPRO) ---
def manhattan_distance(a, b): return abs(a[0] - b[0]) + abs(a[1] - b[1])

class GridMap:
    def __init__(self, width, height):
        self.width, self.height = width, height
        self.walls = set()            # Tembok permanen
        self.closed_doors = set()     # Pintu tertutup (diupdate setiap tick)
        self.movable_blocks = set()   # Balok dorong (koordinatnya bisa berubah)
    ...
    def push_block(self, from_pos, block_pos):
        if block_pos in self.movable_blocks:
            ...
            if not self.is_blocked(push_target):
...
class AdaptiveShadow:
    ...
    def tick(self, grid_map):
        ...
        if self.state == "REPLAYING":
            ...
            grid_map.push_block(self.position, next_pos)
...
class GameState:
    ...
    def move_player(self, dx, dy):
        ...
        self.grid.push_block(self.player, target_pos)
...
def do_action(action_type: str, x: int = 0, y: int = 0):
    ...
    elif action_type == "reset": game.reset() # Fitur Baru
